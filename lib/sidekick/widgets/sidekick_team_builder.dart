...
import 'package:sidekick_with_null_safety/sidekick/widgets/sidekick.dart';
...
class SidekickTeamBuilderState<T> extends State<SidekickTeamBuilder<T>>
  ...
  Future<void> moveAll(SidekickFlightDirection direction) {
    ...
    if (!_allInFlight!) {
      _allInFlight = true;
      final List<_SidekickMission<T>> source = _getSource(direction);
      final List<_SidekickMission<T>> target = _getTarget(direction);
